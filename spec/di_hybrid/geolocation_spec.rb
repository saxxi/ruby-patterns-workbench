describe DiHybrid::GeolocationService::Geolocator do

  subject { ::DiHybrid::GeolocationService::Geolocator.build }
  let(:ip) { 'THE.IP' }
  let(:geo_attributes) {
    {
      city: 'London',
  ...
  describe '#geolocation_from_ip' do
    ...
    it 'returns a geolocation from an ip address' do
      ...
      expect(result).to eq(
        ::DiHybrid::GeolocationService::Entities::Geolocation.new({
          city: 'London',
          region: 'England',
          country_code: 'UK',
        })
